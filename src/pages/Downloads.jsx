...
const Downloads = () => {
  // Static download data - Only KG Admission Form
  const downloads = [
    {
      id: 1,
      title: 'KG ADMISSION FORM 2027-28',
      category: 'Admission',
      file: '/wp-content/uploads/2026/05/KG_APPLICATION_FORM-2027-28.pdf',
      description: 'Application form for Class K.G. 2027-28 admission'
    }
  ]
  ...
  const getFileIcon = (fileUrl) => {
    if (!fileUrl) return <FileText size={20} />
    const ext = fileUrl.split('.').pop()?.toLowerCase()
    if (ext === 'pdf') return <FileCheck size={20} />
            ...
            <div className="space-y-8 max-w-3xl mx-auto">
              {categories.map((category) => (
                <div key={category}>
                  ...
                  <div className="space-y-3">
                    {groupedDownloads[category].map((item) => (
                      <SectionWrapper key={item.id} className="bg-white rounded-xl shadow-sm border border-gray-100">
                        <a
                          href={getFullFileUrl(item.file)}
                          target="_blank"
                          rel="noopener noreferrer"
